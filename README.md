...
生のtool callとは異なり、capabilityは明確な契約を持つ業務操作を表すことを意図しています。現在の実装では、capabilityは`name`、`description`、任意の`version`、`inputSchema`、任意の`outputSchema`、`sideEffect`、`approval`、`audit`、`idempotency`、in-memoryの`run` handlerを定義します。`approval.required`が承認待ちへの一時停止を制御し、`approval.reason`が指定されている場合は`needs_approval`レスポンスの`reason`として返されます。互換性のために従来の`requiresApproval`もまだサポートしていますが、新しいcapabilityでは`approval.required`を使うことを推奨します。
